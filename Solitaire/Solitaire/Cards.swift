...
//  Solitaire
//
//  Created by David Ciupei on 4/27/17.
    ...
    static func deck() -> [Card] {
        ...
        for suit in 0 ... 3  {
            for rank in 1 ... 13 {
                ...
                switch suit {
                case 0:
                    new_Deck = Card(suit: .spades, rank: UInt8(rank))
                case 1:
                    new_Deck = Card(suit: .clubs, rank: UInt8(rank))
                case 2:
                    new_Deck = Card(suit: .diamonds, rank: UInt8(rank))
                case 3:
                    new_Deck = Card(suit: .hearts, rank: UInt8(rank))
                default:
                    break;
                }
